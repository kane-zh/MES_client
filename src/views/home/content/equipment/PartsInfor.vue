  ...
  <div class="partsInfor">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加备品信息</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"备品信息名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"备品信息编码:"+"&#12288;"+detail.code}}</li>
          <li>{{"状态:"+"&#12288;"+detail.state}}</li>
          <li>{{"类型名称:"+"&#12288;"+type.name}}</li>
          <li>{{"类型编码:"+"&#12288;"+type.code}}</li>
          <li>{{"类型状态:"+"&#12288;"+type.state}}</li>
          <li>{{"供应商名称:"+"&#12288;"+vendor.name}}</li>
          <li>{{"供应商编码:"+"&#12288;"+vendor.code}}</li>
          <li>{{"供应商信息状态:"+"&#12288;"+vendor.state}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>备品名称:
          <input v-model="formItem.name"  placeholder="请输入备品名称...">
        </div>
        <div>备品编码:
          <input v-model="formItem.code"  placeholder="请输入备品编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('equipment/partsInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('equipment/partsInfor/?state=' + self.selectItem.state +
    ...
    changeState (state) {
      ...
      this.$axios.patch(`equipment/partsInfor/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`equipment/partsInfor/` + id).then(function (response) {
        ...
        self.formItem.vendor = response.data.vendor
    ...
    save () {
      ...
      this.$axios.post(`equipment/partsInfor/`, {
    ...
    update () {
      ...
      this.$axios.put(`equipment/partsInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        vendor: self.formItem.vendor,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`equipment/partsInfor/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`equipment/partsInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'equipment.add_partsinfordefinitionmodel' in this.permissions
    },
    canRead () {
      return 'equipment.read_partsinfordefinitionmodel' in this.permissions
    }
