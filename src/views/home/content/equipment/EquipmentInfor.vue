  ...
  <div class="equipmentInfor">
    ...
    <div  class="list">
      <div class="heard">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加设备信息</button>
        </div>
      </div>
      <div class="content">
        <div class="table">
      ...
      <div class="center">
        ...
        <div class="content">
          <div class="basic">
            ...
            <dd>{{"厂商:"+"&#12288;"+vendor.name+"("+vendor.code+")"}}</dd>
            <dd>{{"归属单位:"+"&#12288;"+detail.affiliation}}</dd>
            <dd>{{"所在位置:"+"&#12288;"+detail.location}}</dd>
            <dd>{{"负责人:"+"&#12288;"+detail.principal}}</dd>
            <dd>{{"折旧率:"+"&#12288;"+detail.depreciationRate}}</dd>
            <dd>{{"启用日期:"+"&#12288;"+detail.dataOfActivation}}</dd>
            <dd>{{"购买日期:"+"&#12288;"+detail.dataOfPurchase}}</dd>
          ...
          <div class="child">
            ...
            <table >
              ...
              <tr align="center" v-for="(item,index) in detail.parts" :key="item.id" type="height:1em" >
                ...
                <td>{{item.type.name+"("+item.type.code+")"}}</td>
                <td>{{item.state}}</td>
                <td><span @click="showPartsView(item.id)">点击</span></td>
              </tr>
              <tr>
    ...
    <!-- 备品信息显示对话框-->
    <div  class="partsDialog"  v-show="showPartsDialog==='true'">
      <div class="center">
        <div class="heard">
          <span>备品详情信息</span>
          <button type="button" @click="showPartsDialog='false'"></button>
        </div>
        <div class="content">
          <div class="basic">
            <dt>基础信息</dt>
            <dd>{{"名称:"+"&#12288;"+partsDetail.name}}</dd>
            <dd>{{"编码:"+"&#12288;"+partsDetail.code}}</dd>
            <dd>{{"状态:"+"&#12288;"+partsDetail.state}}</dd>
            <dd>{{"分类:"+"&#12288;"+type.name+"("+type.code+")"}}</dd>
            <dd>{{"供应商:"+"&#12288;"+vendor.name+"("+vendor.code+")"}}</dd>
          </div>
          <div class="file">
            <dt>文件附件</dt>
            <dd v-for="(value,id) in partsDetail.file" :key="id">
              <a target='_black' v-bind:key="id" :href="value.file">{{value.file_name}}</a>
            </dd>
          </div>
          <div class="image">
            <dt>图片附件</dt>
            <dd v-for="(value,id) in partsDetail.image" :key="id">
              <a target='_black' v-bind:key="id" :href="value.image"> <img :src="value.image" width="50px"></a>
            </dd>
          </div>
        </div>
  ...
  methods: {
    ...
    showPartsView (id) {
      ...
      this.$axios.get(`equipment/partsInfor/` + id).then(function (response) {
        ...
        self.partsType = self.partsDetail.type
        self.showPartsDialog = 'true'
      }).catch(function (err) {
        // 错误提示
        console.log(err)
      })
