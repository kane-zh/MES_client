<template>
  <div class="equipmentMaintainPlan">
    <!-- 列表页显示-->
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加设备维护计划</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"设备维护计划名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"设备维护计划编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>设备维护计划名称:
          <input v-model="formItem.name"  placeholder="请输入设备维护计划名称...">
        </div>
        <div>设备维护计划编码:
          <input v-model="formItem.code"  placeholder="请输入设备维护计划编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('plan/equipmentMaintainPlan/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('plan/equipmentMaintainPlan/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      var self = this
      this.$axios.get(`plan/equipmentMaintainPlan/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`plan/equipmentMaintainPlan/` + self.detail.id + '/', {
    ...
    save () {
      ...
      this.$axios.post(`plan/equipmentMaintainPlan/`, {
    ...
    update () {
      ...
      this.$axios.put(`plan/equipmentMaintainPlan/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        file: self.formItem.file,
        desc: self.formItem.desc,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`plan/equipmentMaintainPlan/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`plan/equipmentMaintainPlan/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'plan.add_equipmentmaintainplanmodel' in this.permissions
    },
    canRead () {
      return 'plan.read_equipmentmaintainplanmodel' in this.permissions
    }
