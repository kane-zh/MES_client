  ...
  <div class="planBoard">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加计划看板</button>
        </div>
        <div class="ordering">
          <div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('plan/board/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('plan/board/?state=' + self.selectItem.state +
    ...
    showDetailView (value) {
      ...
      this.imageInfor = {}
      var self = this
      if (!(('plan.read_planboardmodel' in this.permissions) || (value.auditor === this.username) || (value.create_user === this.username))) {
        alert('当前账号不具备查看详情权限')
        return
      }
      this.$axios.get(`plan/board/` + value.id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`plan/board/` + self.detail.id + '/', {
    ...
    uploadImage () {
      ...
      var self = this
      this.$axios.post(`plan/image/`, formData,
    ...
    update () {
      ...
      this.$axios.put(`plan/board/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        file: self.formItem.file,
        image: self.formItem.image,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`plan/board/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`plan/board/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'plan.add_planboardmodel' in this.permissions
    },
    canRead () {
      return 'plan.read_planboardmodel' in this.permissions
    }
