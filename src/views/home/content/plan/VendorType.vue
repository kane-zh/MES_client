<template>
  <div class="vendorType">
      ...
      <div v-if="showTree===false">
        <div class="listHead">
          ...
          <div class="button" >
            ...
            <button type="button" @click="showCreatView"  v-show="canCreate===true">添加设备供应商类型</button>
          </div>
          <div class="ordering">
            <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"设备供应商类型名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"设备供应商类型编码:"+"&#12288;"+detail.code}}</li>
        ...
        <Collapse active-key="2" accordion v-if="detail.vendorType_child!==undefined && detail.vendorType_child.length > 0">
          子类别:
          <Panel >
            <div  slot="content">
              <table >
                ...
                <tr align="center" v-for="(item,index) in detail.vendorType_child" :key="item.id" type="height:1em" >
                  <td>{{index}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.code}}</td>
                  <td>{{item.classes}}</td>
        ...
        <Collapse active-key="3" accordion v-if="detail.vendorType_item!==undefined && detail.vendorType_item.length > 0">
          包含内容:
          <Panel >
            <div  slot="content">
              <table >
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
          <div>类型名称:
            <input v-model="formItem.name"  placeholder="请输入供应商类型名称...">
          </div>
          <div>类型编码:
            <input v-model="formItem.code"  placeholder="请输入供应商类型编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('plan/vendorType/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('plan/vendorType/?state=' + self.selectItem.state +
