  ...
  <div class="purchaseRequire">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加物料需求计划</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div>
        <ul>
          <li>{{"物料采购计划名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"物料采购计划编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>物料采购计划名称:
          <input v-model="formItem.name"  placeholder="请输入物料采购计划名称...">
        </div>
        <div>物料采购计划编码:
          <input v-model="formItem.code"  placeholder="请输入物料采购计划编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('plan/purchaseRequire/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('plan/purchaseRequire/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      var self = this
      this.$axios.get(`plan/purchaseRequire/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`plan/purchaseRequire/` + self.detail.id + '/', {
    ...
    save () {
      ...
      this.$axios.post(`plan/purchaseRequire/`, {
    ...
    update () {
      ...
      this.$axios.put(`plan/purchaseRequire/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        file: self.formItem.file,
        desc: self.formItem.desc,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`plan/purchaseRequire/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`plan/purchaseRequire/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'plan.add_purchaserequirecreatemodel' in this.permissions
    },
    canRead () {
      return 'plan.read_purchaserequirecreatemodel' in this.permissions
    }
