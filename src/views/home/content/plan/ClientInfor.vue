  ...
  <div class="clientInfor">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加客户信息</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"客户信息名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"客户信息编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>客户名称:
          <input v-model="formItem.name"  placeholder="请输入客户名称...">
        </div>
        <div>客户编码:
          <input v-model="formItem.code"  placeholder="请输入客户编码...">
...
export default {
  ...
  methods: {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('plan/clientInfor/?state=' + self.selectItem.state +
    ...
    save () {
      ...
      this.$axios.post(`plan/clientInfor/`, {
    ...
    update () {
      ...
      this.$axios.put(`plan/clientInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        address: self.formItem.address,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`plan/clientInfor/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`plan/clientInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  created () {
    ...
    this.$axios.get('user/userInfor/?page_size=99999&ordering=-id').then(function (response) {
      ...
      self.$axios.get('plan/clientType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
  ...
  computed: {
    ...
    canCreate () {
      return 'plan.add_clientinfordefinitionmodel' in this.permissions
    },
    canRead () {
      return 'plan.read_clientinfordefinitionmodel' in this.permissions
    }
  ...
  watch: {
    ...
    'formItem.type': function (newval, oldval) {
      ...
      this.$axios.get(`plan/clientType/` + newval).then(function (response) {
