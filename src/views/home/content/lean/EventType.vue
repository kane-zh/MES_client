  ...
  <div class="eventType">
      ...
      <div v-if="showTree===false">
        <div class="listHead">
          ...
          <div class="button" >
            ...
            <button type="button" @click="showCreatView"  v-show="canCreate===true">添加事件类型</button>
          </div>
          <div class="ordering">
            <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        ...
        <Collapse active-key="2" accordion v-if="detail.eventType_child!==undefined && detail.eventType_child.length > 0">
          子类别:
          <table >
            <tr align="center"  type="height:2em">
              <th>序号</th>
            ...
            <tr align="center" v-for="(item,index) in detail.eventType_child" :key="item.id" type="height:1em" >
              <td>{{index}}</td>
              <td>{{item.name}}</td>
              <td>{{item.code}}</td>
              <td>{{item.classes}}</td>
        ...
        <Collapse active-key="3" accordion v-if="detail.eventType_item!==undefined && detail.eventType_item.length > 0">
          包含内容:
          <Panel >
            <div  slot="content">
              <table >
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('lean/eventType/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('lean/eventType/?state=' + self.selectItem.state +
    ...
    showListTreeView: function () {
      this.treeData = []
      var self = this
      this.$axios.get('lean/eventTypes/').then(function (response) {
    ...
    transformToTree (data) {
      ...
      for (let l1 = 0; l1 < data.length; l1++) {
        /* 遍历第二层,给第一层子项添加数据 */
        let data2 = data[l1].eventType_child
        let children1 = []
        for (let l2 = 0; l2 < data2.length; l2++) {
          /* 遍历第三层,给第二层子项添加数据 */
          let data3 = data2[l2].eventType_child
          let children2 = []
          for (let l3 = 0; l3 < data3.length; l3++) {
            /* 遍历第四层,给第三层子项添加数据 */
            let data4 = data3[l3].eventType_child
            let children3 = []
    ...
    getTreeIndex () {
      ...
      if (!(('lean.read_eventtypedefinitionmodel' in this.permissions) || (data[0].auditor === this.username) || (data[0].create_user === this.username))) {
        alert('当前账号不具备查看详情权限')
        return
      }
      if (data[0].state !== '使用中') {
    ...
    showDetailView (id) {
      ...
      this.parent = {}
      var self = this
      this.$axios.get(`lean/eventType/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`lean/eventType/` + self.detail.id + '/', {
    ...
    save () {
      ...
      this.$axios.post(`lean/eventType/`, {
    ...
    update () {
      ...
      this.$axios.put(`lean/eventType/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        classes: self.formItem.classes,
        parent: self.formItem.parent,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`lean/eventType/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`lean/eventType/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  mounted () {
    this.attach_attributes = this.$store.getters.getConfig.attach_attribute.事件信息
  },
  computed: {
    username () {
    ...
    canCreate () {
      return 'lean.add_eventtypedefinitionmodel' in this.permissions
    },
    canRead () {
      return 'lean.read_eventtypedefinitionmodel' in this.permissions
    }
    ...
    'formItem.classes': function (newval, oldval) {
      ...
      switch (newval) {
        case '一级类别':
          ...
          break
        case '二级类别':
          this.$axios.get('lean/eventType/?state=使用中&classes=一级类别&auditor=&create_user=&page_size=99999&ordering=-id').then(function (response) {
            self.list = response.data.results
          }).catch(function (err) {
            if (err.request) {
              alert(err.request.response)
          ...
          break
        case '三级类别':
          this.$axios.get('lean/eventType/?state=使用中&classes=二级类别&auditor=&create_user=&page_size=99999&ordering=-id').then(function (response) {
            self.list = response.data.results
          }).catch(function (err) {
            if (err.request) {
              alert(err.request.response)
          ...
          break
        case '四级类别':
          this.$axios.get('lean/eventType/?state=使用中&classes=三级类别&auditor=&create_user=&page_size=99999&ordering=-id').then(function (response) {
            self.list = response.data.results
          }).catch(function (err) {
            if (err.request) {
              alert(err.request.response)
