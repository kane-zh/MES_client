  ...
  <div class="skillInfor">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加技能信息</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>技能名称:
          <input v-model="formItem.name"  placeholder="请输入技能名称...">
        </div>
        <div>技能编码:
          <input v-model="formItem.code"  placeholder="请输入技能编码...">
        ...
        <div>鉴定规则:
          <textarea v-model="formItem.rule"  placeholder="请输入鉴定规则..."></textarea>
          <span class="message" v-if="!$v.formItem.rule.required">请输入鉴定规则</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/skillInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    showDetailView (id) {
      ...
      this.type = {}
      var self = this
      this.$axios.get(`production/skillInfor/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`production/skillInfor/` + self.detail.id + '/', {
    ...
    save () {
      ...
      this.$axios.post(`production/skillInfor/`, {
    ...
    update () {
      ...
      this.$axios.put(`production/skillInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        rule: self.formItem.rule,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`production/skillInfor/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`production/skillInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  created () {
    ...
    this.$axios.get('user/userInfor/?page_size=99999&ordering=-id').then(function (response) {
      ...
      self.$axios.get('production/skillType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
  ...
  computed: {
    ...
    canCreate () {
      return 'production.add_skillinfordefinitionmodel' in this.permissions
    },
    canRead () {
      return 'production.read_skillinfordefinitionmodel' in this.permissions
    }
  ...
  watch: {
    ...
    'formItem.type': function (newval, oldval) {
      ...
      this.$axios.get(`production/skillType/` + newval).then(function (response) {
