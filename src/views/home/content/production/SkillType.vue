<template>
  <div class="skillType">
      ...
      <div v-if="showTree===false">
        <div class="listHead">
          ...
          <div class="button" >
            ...
            <button type="button" @click="showCreatView"  v-show="canCreate===true">添加技能类型</button>
          </div>
          <div class="ordering">
            <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"技能类型名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"技能类型编码:"+"&#12288;"+detail.code}}</li>
        ...
        <Collapse active-key="2" accordion v-if="detail.skillType_child!==undefined && detail.skillType_child.length > 0">
          子类别:
          <Panel >
            <div  slot="content">
              <table >
                ...
                <tr align="center" v-for="(item,index) in detail.skillType_child" :key="item.id" type="height:1em" >
                  <td>{{index}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.code}}</td>
                  <td>{{item.classes}}</td>
        ...
        <Collapse active-key="3" accordion v-if="detail.skillType_item!==undefined && detail.skillType_item.length > 0">
          包含内容:
          <Panel >
            <div  slot="content">
              <table >
                ...
                <tr align="center" v-for="(item,index) in detail.skillType_item" :key="item.id" type="height:1em" >
                  <td>{{index}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.code}}</td>
                  <td>{{item.state}}</td>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
          <div>类型名称:
            <input v-model="formItem.name"  placeholder="请输入技能类型名称...">
          </div>
          <div>类型编码:
            <input v-model="formItem.code"  placeholder="请输入技能类型编码...">
...
export default {
  ...
  methods: {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('production/skillType/?state=' + self.selectItem.state +
    ...
    transformToTree (data) {
      ...
      for (let l1 = 0; l1 < data.length; l1++) {
        /* 遍历第二层,给第一层子项添加数据 */
        let data2 = data[l1].skillType_child
        let children1 = []
        for (let l2 = 0; l2 < data2.length; l2++) {
          /* 遍历第三层,给第二层子项添加数据 */
          let data3 = data2[l2].skillType_child
          let children2 = []
          for (let l3 = 0; l3 < data3.length; l3++) {
            /* 遍历第四层,给第三层子项添加数据 */
            let data4 = data3[l3].skillType_child
            let children3 = []
  ...
  mounted () {
    this.attach_attributes = this.$store.getters.getConfig.attach_attribute.技能信息
  },
  computed: {
    username () {
