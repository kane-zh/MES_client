  ...
  <div class="productionBoard">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加生产看板</button>
        </div>
        <div class="ordering">
          <div>
      ...
      <div class="listTable">
        <div class="board" v-for="item in list" :value="item.name" :key="item.code">
          ...
          <div class="operationbutton">
            ...
            <button type="button" @click="showUpdateView(item)" v-if="item.state==='新建'">更改</button>
          </div>
        </div>
        <div class="page">
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"看板名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"看板编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>看板名称:
          <input v-model="formItem.name"  placeholder="请输入看板名称...">
        </div>
        <div>看板编码:
          <input v-model="formItem.code"  placeholder="请输入看板编码...">
        ...
        <div class="annex">图片附件:
          <img :src="formItem.image" width="50px">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/board/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('production/board/?state=' + self.selectItem.state +
    ...
    showDetailView (value) {
      ...
      this.imageInfor = {}
      var self = this
      if (!(('production.read_productionboardmodel' in this.permissions) || (value.auditor === this.username) || (value.create_user === this.username))) {
        alert('当前账号不具备查看详情权限')
        return
      }
      this.$axios.get(`production/board/` + value.id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`production/board/` + self.detail.id + '/', {
    ...
    showFile (value) {
      ...
      this.$axios.get(`production/board/` + value.id).then(function (response) {
        response.data.file.forEach(function (value, i) {
          window.open(value.file, value.file_name)
        })
      }).catch(function (err) {
        if (err.request) {
          alert(err.request.response)
    ...
    save () {
      ...
      this.$axios.post(`production/board/`, {
    ...
    update () {
      ...
      this.$axios.put(`production/board/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        file: self.formItem.file,
        image: self.formItem.image,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`production/board/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`production/board/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'production.add_productionboardmodel' in this.permissions
    },
    canRead () {
      return 'production.read_productionboardmodel' in this.permissions
    }
...
<style scoped>
  ...
  .list .listTable .board img{
    position: absolute;
    top: 0;
    height: 70%;
    width: 100%;
  }
  .list .listTable .board span{
    position: absolute;
    top: 70%;
    height: 15%;
    width: 100%;
  ...
  .list .listTable .board .operationbutton{
    position: absolute;
    top: 85%;
    height: 15%;
    margin-left: 40%;
  ...
  .list .listTable .board .operationbutton button{
    width: 4em;
    background: #ffffff;
    border: 1px solid #363E42;
    border-radius: 13px;
