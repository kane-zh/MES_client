<template>
  <div class="workshopInfor">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      ...
      <div class="listTree" v-if="showTree===true">
        ...
        <div class="button">
          ...
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加车间</button>
        </div>
      </div>
    </div>
    <!--   /*详情页显示*/-->
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"车间名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"车间编码:"+"&#12288;"+detail.code}}</li>
        ...
        <Collapse active-key="2" accordion v-if="detail.workshopInfor_child!==undefined && detail.workshopInfor_child.length > 0">
          子类别:
          <Panel >
            <div  slot="content">
              <table >
                ...
                <tr align="center" v-for="(item,index) in detail.workshopInfor_child" :key="item.id" type="height:1em" >
                  <td>{{index}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.code}}</td>
                  <td>{{item.classes}}</td>
        ...
        <Collapse active-key="3" accordion v-if="detail.workshopInfor_item!==undefined && detail.workshopInfor_item.length > 0">
          包含内容:
          <Panel >
            <div  slot="content">
              <table >
                ...
                <tr align="center" v-for="(item,index) in detail.workshopInfor_item" :key="item.id" type="height:1em" >
                  <td>{{index}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.code}}</td>
                  <td>{{item.state}}</td>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
          <div>车间名称:
            <input v-model="formItem.name"  placeholder="请输入车间名称...">
          </div>
          <div>车间编码:
            <input v-model="formItem.code"  placeholder="请输入车间编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/workshopInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('production/workshopInfor/?state=' + self.selectItem.state +
