...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/productDailyReport/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('production/productDailyReport/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      this.list_child = []
      var self = this
      this.$axios.get(`production/productDailyReport/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`production/productDailyReport/` + self.detail.id + '/', {
    ...
    uploadChild () {
      var self = this
      this.$axios.post(`production/productDailyReportItem/`, {
    ...
    save () {
      ...
      this.$axios.post(`production/productDailyReport/`, {
    ...
    update () {
      ...
      this.$axios.put(`production/productDailyReport/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        dataTime: self.formItem.dataTime,
        team: self.formItem.team,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`production/productDailyReport/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`production/productDailyReport/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  mounted () {
    this.attribute_title = this.$store.getters.getConfig.attach_attribute.产品生产日报记录
    this.attribute_title_child = this.$store.getters.getConfig.attach_attribute.产品生产日报子项
  },
  computed: {
    username () {
    ...
    canCreate () {
      return 'production.add_productdailyreportmodel' in this.permissions
    },
    canRead () {
      return 'production.read_productdailyreportmodel' in this.permissions
    }
