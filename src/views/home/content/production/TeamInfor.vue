<template>
  <div class="teamInfor">
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"班组信息名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"班组信息编码:"+"&#12288;"+detail.code}}</li>
          <li>{{"状态:"+"&#12288;"+detail.state}}</li>
          <li>{{"车间名称:"+"&#12288;"+type.name}}</li>
          <li>{{"车间编码:"+"&#12288;"+type.code}}</li>
          <li>{{"车间信息状态:"+"&#12288;"+type.state}}</li>
          <li>{{"负责人:"+"&#12288;"+detail.principal}}</li>
          <li>{{"职责:"+"&#12288;"+detail.duties}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>班组名称:
          <input v-model="formItem.name"  placeholder="请输入班组名称...">
        </div>
        <div>班组编码:
          <input v-model="formItem.code"  placeholder="请输入班组编码...">
        ...
        <select v-model="formItem.type"   placeholder="请选择车间">
          ...
          <span class="message" v-if="!$v.formItem.type.required">请选择车间</span>
        </div>
        <div>负责人:
          <input v-model="formItem.principal"  placeholder="请输入班组负责人...">
        </div>
        <div>职责:
          <textarea v-model="formItem.duties" placeholder="请输入班组职责..."></textarea>
        </div>
        <div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/teamInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('production/teamInfor/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      this.type = {}
      var self = this
      this.$axios.get(`production/teamInfor/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`production/teamInfor/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`production/teamInfor/` + id).then(function (response) {
        ...
        self.formItem.duties = response.data.duties
    ...
    update () {
      ...
      this.$axios.put(`production/teamInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        principal: self.formItem.principal,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`production/teamInfor/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`production/teamInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'production.add_teaminfordefinitionmodel' in this.permissions
    },
    canRead () {
      return 'production.read_teaminfordefinitionmodel' in this.permissions
    }
