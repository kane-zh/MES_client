  ...
  <div class="assessmentLevel">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加考核等级</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"考核等级名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"考核等级编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>考核等级名称:
          <input v-model="formItem.name"  placeholder="请输入考核等级名称...">
        </div>
        <div>考核等级编码:
          <input v-model="formItem.code"  placeholder="请输入考核等级编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/assessmentLevel/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('production/assessmentLevel/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      var self = this
      this.$axios.get(`production/assessmentLevel/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`production/assessmentLevel/` + self.detail.id + '/', {
    ...
    save () {
      ...
      this.$axios.post(`production/assessmentLevel/`, {
    ...
    update () {
      ...
      this.$axios.put(`production/assessmentLevel/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        rule: self.formItem.rule,
        file: self.formItem.file,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`production/assessmentLevel/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`production/assessmentLevel/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'production.add_assessmentleveldefinitionmodel' in this.permissions
    },
    canRead () {
      return 'production.read_assessmentleveldefinitionmodel' in this.permissions
    }
