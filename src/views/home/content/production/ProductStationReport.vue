<template>
    <div class="ProductStationReport">
      <!-- 列表页显示-->
      <div  class="list">
        <div class="heard">
          <form class="select">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('production/productStationReport/?ordering=' + self.selectItem.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('production/productStationReport/?productType_code=' + self.selectItem.productType +
  ...
  mounted () {
    this.attribute_title = this.$store.getters.getConfig.attach_attribute.产品工序报工
  },
  watch: {
    selectItem: {
      deep: true
