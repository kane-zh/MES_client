<template>
  <div class="productRouteInfor">
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"生产线路信息名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"生产线路信息编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>生产线路名称:
          <input v-model="formItem.name"  placeholder="请输入生产线路名称...">
        </div>
        <div>生产线路编码:
          <input v-model="formItem.code"  placeholder="请输入生产线路编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('process/productRoute/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('process/productRoute/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      this.list_child = []
      var self = this
      this.$axios.get(`process/productRoute/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`process/productRoute/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`process/productRoute/` + id).then(function (response) {
        ...
        response.data.file.forEach(function (value, i) {
          ...
          self.fileData.push(obj1)
        })
        response.data.station.forEach(function (value, i) {
    ...
    save () {
      ...
      this.$axios.post(`process/productRoute/`, {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        station: self.formItem.child,
    ...
    update () {
      ...
      this.$axios.put(`process/productRoute/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        station: self.formItem.child,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`process/productRoute/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`process/productRoute/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  created () {
    ...
    this.$axios.get('user/userInfor/?page_size=99999&ordering=-id').then(function (response) {
      ...
      self.$axios.get('process/productRouteType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
        ...
        self.$axios.get('process/stationInfor/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
  ...
  computed: {
    ...
    canCreate () {
      return 'process.add_productroutedefinitionmodel' in this.permissions
    },
    canRead () {
      return 'process.read_productroutedefinitionmodel' in this.permissions
    }
