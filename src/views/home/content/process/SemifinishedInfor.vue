  ...
  <div class="semifinishedInfor">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加半成品信息</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"半成品信息名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"半成品信息编码:"+"&#12288;"+detail.code}}</li>
          <li>{{"状态:"+"&#12288;"+detail.state}}</li>
          <li>{{"类型名称:"+"&#12288;"+type.name}}</li>
          <li>{{"类型编码:"+"&#12288;"+type.code}}</li>
          <li>{{"类型状态:"+"&#12288;"+type.state}}</li>
          <li>{{"单位名称:"+"&#12288;"+unit.name}}</li>
          <li>{{"单位编码:"+"&#12288;"+unit.code}}</li>
          <li>{{"单位状态:"+"&#12288;"+unit.state}}</li>
          <li>{{"生产线路类型名称:"+"&#12288;"+detail.routeType_name}}</li>
          <li>{{"生产线路类型编码:"+"&#12288;"+detail.routeType_code}}</li>
          <li>{{"生产线路名称:"+"&#12288;"+detail.route_name}}</li>
          <li>{{"生产线路编码:"+"&#12288;"+detail.route_code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>半成品名称:
          <input v-model="formItem.name"  placeholder="请输入半成品名称...">
        </div>
        <div>半成品编码:
          <input v-model="formItem.code"  placeholder="请输入半成品编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('process/semifinishedInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    showDetailView (id) {
      ...
      this.type = {}
      var self = this
      var jsonObj = {'type': 'semifinishedInfor'}
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`process/semifinishedInfor/` + id).then(function (response) {
        ...
        for (var i = 0; i < self.productRouteType; i++) {
    ...
    save () {
      ...
      this.$axios.post(`process/semifinishedInfor/`, {
    ...
    update () {
      ...
      this.$axios.put(`process/semifinishedInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        file: self.formItem.file,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`process/semifinishedInfor/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`process/semifinishedInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  watch: {
    ...
    'formItem.productRouteType': function (newval, oldval) {
      ...
      this.$axios.get(`process/productRouteType/` + newval).then(function (response) {
        self.productRouteInfor = response.data.productRouteType_item
      }).catch(function (err) {
        if (err.request) {
          alert(err.request.response)
        } else {
