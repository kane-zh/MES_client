<template>
  <div class="unitInfor">
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"计量单位名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"计量单位编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>计量单位名称:
          <input v-model="formItem.name"  placeholder="请输入计量单位名称...">
        </div>
        <div>计量单位编码:
          <input v-model="formItem.code"  placeholder="请输入计量单位编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('process/unitInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('process/unitInfor/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      this.type = {}
      var self = this
      this.$axios.get(`process/unitInfor/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`process/unitInfor/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`process/unitInfor/` + id).then(function (response) {
        ...
        self.formItem.symbol = response.data.symbol
    ...
    save () {
      ...
      this.$axios.post(`process/unitInfor/`, {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        symbol: self.formItem.symbol,
    ...
    update () {
      ...
      this.$axios.put(`process/unitInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        symbol: self.formItem.symbol,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`process/unitInfor/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`process/unitInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'process.add_unitinfordefinitionmodel' in this.permissions
    },
    canRead () {
      return 'process.read_unitinfordefinitionmodel' in this.permissions
    }
