<template>
  <div class="stationInfor">
          ...
          <div class="child">
            ...
            <table v-show="list_material.length>0">
              ...
              <tr align="center" v-for="(item,index) in list_material" :key="item.id" >
                <td>{{index}}</td>
                <td>{{item.materialTypeCode+"("+item.materialTypeName+")"}}</td>
      ...
      <div class="center">
        ...
        <div class="content">
          <form >
            ...
            <div class="child">物料:
              <span @click="showChildForm='material'">
                  添加
              </span>
              <div>
                <table v-show="list_material.length>0">
                  ...
                  <tr align="center" v-for="(item,index) in list_material" :key="item.id" >
                    ...
                    <td>{{item.desc}}</td>
                    <td>
                      <button type="button" @click="removeMaterial(item.id)">移除</button>
                    </td>
                  </tr>
                  <tr>
            ...
            <div class="child">半成品:
              <span @click="showChildForm='semifinished'">
                  添加
              </span>
              <div>
                <table v-show="list_semifinished.length>0">
                  ...
                  <tr align="center" v-for="(item,index) in list_semifinished" :key="item.id" >
                    ...
                    <td>{{item.desc}}</td>
                    <td>
                      <button type="button" @click="removeSemifinished(item.id)">移除</button>
                    </td>
                  </tr>
                  <tr>
    ...
    <!--    /*子项添加对话框*/-->
    <div class="dialog" v-show ="showChildForm==='material'">
      <div class="center">
        <div class="heard">
          <span>子项添加</span>
          ...
          <form>
            <div>物料分类:
              <select v-model="formItem_material.materialType" >
                <option v-for="item in materialType" :value="item.id" :key="item.id">{{item.name +"("+ item.name+")"}}</option>
              </select>
            </div>
            <div>物料信息:
              <select v-model="formItem_material.material" >
                <option v-for="item in materialInfor" :value="item.id" :key="item.id">{{item.name + "("+item.name+")"}}</option>
              </select>
            </div>
            <div>数量:
              <input type="number" v-model="formItem_material.sum">
            </div>
            <div>备注信息:
              <textarea v-model="formItem_material.desc" placeholder="请输入当前物料说明信息..."></textarea>
            </div>
            <div>
              <button type="button" @click="uploadMaterial">添加</button>
            </div>
          </form>
        </div>
      </div>
    </div>
    <div class="dialog" v-show ="showChildForm==='semifinished'">
      <div class="center">
        <div class="heard">
          <span>子项添加</span>
          <button type="button" @click="showChildForm=''"></button>
        ...
        <div class="content">
          <form>
            ...
            <div>半成品信息:
              <select v-model="formItem_semifinished.semifinished" >
                <option v-for="item in semifinishedInfor" :value="item.id" :key="item.id">{{item.name +"("+ item.name+")"}}</option>
              </select>
            </div>
            <div>数量:
              <input type="number" v-model="formItem_semifinished.sum">
            </div>
            <div>备注信息:
              <textarea v-model="formItem_semifinished.desc" placeholder="请输入当前半成品说明信息..."></textarea>
            </div>
            <div>
              <button type="button" @click="uploadSemifinished">添加</button>
            </div>
          </form>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('process/stationInfor/?state=' + self.selectItem.state +
    ...
    showCreatView () {
      ...
      /* 清空子项表单数据 */
      for (let key in this.formItem_material) {
        if (Object.prototype.toString.call(this.formItem_material[key]) === '[object Array]') {
          this.formItem_material[key] = []
        } else if (Object.prototype.toString.call(this.formItem_material[key]) === '[object Object]') {
          var obj1 = this.formItem_material[key]
      ...
      /* 清空子项表单数据 */
      for (let key in this.formItem_semifinished) {
        if (Object.prototype.toString.call(this.formItem_semifinished[key]) === '[object Array]') {
          this.formItem_semifinished[key] = []
        } else if (Object.prototype.toString.call(this.formItem_semifinished[key]) === '[object Object]') {
          var obj2 = this.formItem_semifinished[key]
    ...
    uploadSemifinished () {
      ...
      this.$axios.post(`process/stationSemifinished/`, {
        ...
        for (var i = 0; i < self.semifinishedInfor.length; i++) {
          if (self.semifinishedInfor[i].id === response.data.semifinished) {
            index = i
            break
          }
        }
        ...
        for (var j = 0; j < self.semifinishedType.length; j++) {
          if (self.semifinishedType[j].id === self.formItem_semifinished.semifinishedType) {
            index1 = j
            break
          }
        }
    ...
    removeSemifinished: function (id) {
      var self = this
      if (!confirm('是否要删除当前数据' + id)) {
        // 当用户点击的取消按钮的时候，应该阻断这个方法中的后面代码的继续执行
        return
  ...
  watch: {
    ...
    'formItem_material.materialType': function (newval, oldval) {
      ...
      this.$axios.get('process/materialInfor/?page_size=99999&ordering=-id&state=使用中' +
        ...
        self.materialInfor = response.data.results
      }).catch(function (err) {
        // 错误提示
        console.log(err)
      })
