  ...
  <div class="qualityBoard">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加品质看板</button>
        </div>
        <div class="ordering">
          <div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('quality/board/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('quality/board/?state=' + self.selectItem.state +
    ...
    showDetailView (value) {
      ...
      this.imageInfor = {}
      var self = this
      if (!(('quality.read_qualityboardmodel' in this.permissions) || (value.auditor === this.username) || (value.create_user === this.username))) {
        alert('当前账号不具备查看详情权限')
        return
      }
      this.$axios.get(`quality/board/` + value.id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`quality/board/` + self.detail.id + '/', {
    ...
    update () {
      ...
      this.$axios.put(`quality/board/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        file: self.formItem.file,
        image: self.formItem.image,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`quality/board/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`quality/board/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'quality.add_qualityboardmodel' in this.permissions
    },
    canRead () {
      return 'quality.read_qualityboardmodel' in this.permissions
    }
