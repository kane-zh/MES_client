<template>
  <div class="inspectionStandard">
      ...
      <div class="center">
        ...
        <div class="content">
          <div class="basic">
            ...
            <dd>{{"让步接收比例:"+"&#12288;"+detail.concession_ration}}</dd>
    ...
    <!-- 缺陷信息显示对话框-->
    <div  class="defectDialog"  v-show="showDefectDialog==='true'">
      <div class="center">
        <div class="heard">
          <span>缺陷详情信息</span>
          <button type="button" @click="showDefectDialog='false'"></button>
        </div>
        <div class="content">
          <div class="basic">
            <dt>基础信息</dt>
          ...
          <div class="file">
            <dt>文件附件</dt>
            <dd v-for="(value,id) in defectDetail.file" :key="id">
              <a target='_black' v-bind:key="id" :href="value.file">{{value.file_name}}</a>
            </dd>
          </div>
          <div class="image">
            <dt>图片附件</dt>
            <dd v-for="(value,id) in defectDetail.image" :key="id">
  ...
  methods: {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`quality/inspectionStandardInfor/` + id).then(function (response) {
        ...
        response.data.defect.forEach(function (value, i) {
          self.formItem.defect.push(value.id)
        })
    ...
    showDefectView (id) {
      ...
      this.$axios.get(`quality/defectInfor/` + id).then(function (response) {
        ...
        self.defectGrade = self.defectDetail.defect_grade
        self.defectType = self.defectDetail.type
        self.showDefectDialog = 'true'
      }).catch(function (err) {
        // 错误提示
        console.log(err)
      })
