  ...
  <div class="inspectionStandard">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加检验标准</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"检验标准名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"检验标准编码:"+"&#12288;"+detail.code}}</li>
          <li>{{"状态:"+"&#12288;"+detail.state}}</li>
          <li>{{"类型名称:"+"&#12288;"+type.name}}</li>
          <li>{{"类型编码:"+"&#12288;"+type.code}}</li>
          <li>{{"类型状态:"+"&#12288;"+type.state}}</li>
          <li>{{"抽检比例:"+"&#12288;"+detail.samples_ration}}</li>
          <li>{{"合格比例:"+"&#12288;"+detail.ok_ration}}</li>
          <li>{{"不合格比例:"+"&#12288;"+detail.ng_ration}}</li>
          <li>{{"让步接收比例:"+"&#12288;"+detail.concession_ration}}</li>
        ...
        <Collapse active-key="2" accordion v-if="detail.defect!==undefined && detail.defect.length > 0">
          缺陷项:
          <Panel >
            <div  slot="content">
              <table >
                ...
                <tr align="center" v-for="(item,index) in detail.defect" :key="item.id" type="height:1em" >
                  <td>{{index}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.code}}</td>
                  <td>{{item.state}}</td>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>检验标准名称:
          <input v-model="formItem.name"  placeholder="请输入检验标准名称...">
        </div>
        <div>检验标准编码:
          <input v-model="formItem.code"  placeholder="请输入检验标准编码...">
        ...
        <div>抽检比例:
          <input v-model="formItem.samples_ration" type="number" >
        </div>
        <div>合格比例:
          <input v-model="formItem.ok_ration" type="number" >
        </div>
        <div>不合格比例:
          <input v-model="formItem.ng_ration" type="number" >
        </div>
        <div>让步接收比例:
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('quality/inspectionStandardInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`quality/inspectionStandardInfor/` + id).then(function (response) {
        ...
        self.formItem.samples_ration = response.data.samples_ration
        self.formItem.ok_ration = response.data.ok_ration
        self.formItem.ng_ration = response.data.ng_ration
        self.formItem.concession_ration = response.data.concession_ration
        ...
        response.data.defect.forEach(function (value, i) {
          self.formItem.file.defect(value.id)
        })
    ...
    save () {
      ...
      this.$axios.post(`quality/inspectionStandardInfor/`, {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        file: self.formItem.file,
        defect: self.formItem.defect,
        samples_ration: self.formItem.samples_ration,
        ok_ration: self.formItem.ok_ration,
        ng_ration: self.formItem.ng_ration,
        concession_ration: self.formItem.concession_ration,
    ...
    update () {
      ...
      this.$axios.put(`quality/inspectionStandardInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        file: self.formItem.file,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`quality/inspectionStandardInfor/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`quality/inspectionStandardInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
