  ...
  <div class="defectInfor">
    ...
    <div  class="list">
      <div class="heard">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加缺陷信息</button>
        </div>
      </div>
      <div class="content">
        <div class="table">
      ...
      <div class="center">
        ...
        <div class="content">
          <form >
            ...
            <div>不良等级:
              <select v-model="formItem.defect_grade"   placeholder="请选择不良等级">
                <option v-for="item in gradeInfor" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
              </select>
              <span class="message" v-show ="!$v.formItem.defect_grade.required">请选择不良等级</span>
            </div>
            <div>认证规则:
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('quality/defectInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    showDetailView (id) {
      ...
      this.$axios.get(`quality/defectInfor/` + id).then(function (response) {
        ...
        self.defectGrade = self.detail.defect_grade
    ...
    changeState (state) {
      ...
      this.$axios.patch(`quality/defectInfor/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`quality/defectInfor/` + id).then(function (response) {
        ...
        if (response.data.defect_grade !== null) {
          self.formItem.defect_grade = response.data.defect_grade.id
        } else { self.formItem.defect_grade = response.data.defect_grade }
    ...
    uploadImage () {
      ...
      var self = this
      this.$axios.post(`quality/image/`, formData,
    ...
    save () {
      ...
      this.$axios.post(`quality/defectInfor/`, {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        defect_grade: self.formItem.defect_grade,
    ...
    update () {
      ...
      this.$axios.put(`quality/defectInfor/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        defect_grade: self.formItem.defect_grade,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`quality/defectInfor/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`quality/defectInfor/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  created () {
    ...
    this.$axios.get('user/userInfor/?page_size=99999&ordering=-id').then(function (response) {
      ...
      self.$axios.get('quality/defectType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
        ...
        self.$axios.get('quality/defectGrade/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
          self.gradeInfor = response.data.results
          self.showListView()
        }).catch(function (err) {
          // 错误提示
          console.log(err)
  ...
  computed: {
    ...
    canCreate () {
      return 'quality.add_defectinfordefinitionmodel' in this.permissions
    },
    canRead () {
      return 'quality.read_defectinfordefinitionmodel' in this.permissions
    }
