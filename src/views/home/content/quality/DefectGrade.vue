  ...
  <div class="defectGrade">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加缺陷等级</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"缺陷等级名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"缺陷等级编码:"+"&#12288;"+detail.code}}</li>
          <li>{{"状态:"+"&#12288;"+detail.state}}</li>
          <li>{{"认证规则:"+"&#12288;"+detail.rule}}</li>
          <li>{{"不良影响:"+"&#12288;"+detail.influences}}</li>
          <li>{{"处理方式:"+"&#12288;"+detail.processing_method}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>缺陷等级名称:
          <input v-model="formItem.name"  placeholder="请输入缺陷等级名称...">
        </div>
        <div>缺陷等级编码:
          <input v-model="formItem.code"  placeholder="请输入缺陷等级编码...">
        ...
        <div>认证规则:
          <textarea v-model="formItem.rule"  placeholder="请输入认证规则..."></textarea>
          <span class="message" v-if="!$v.formItem.rule.required">请输入认证规则</span>
        </div>
        <div>不良影响:
          <textarea v-model="formItem.influences"  placeholder="请输入此等级不良影响..."></textarea>
          <span class="message" v-if="!$v.formItem.influences.required">请输入不良影响</span>
        </div>
        <div>处理方式:
          <textarea v-model="formItem.processing_method"  placeholder="请输入此等级下检验品处理方式..."></textarea>
          <span class="message" v-if="!$v.formItem.processing_method.required">请输入处理方式</span>
        </div>
        <div>审核账号:
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('quality/defectGrade/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('quality/defectGrade/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      var self = this
      this.$axios.get(`quality/defectGrade/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`quality/defectGrade/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`quality/defectGrade/` + id).then(function (response) {
        ...
        self.formItem.rule = response.data.rule
        self.formItem.influences = response.data.influences
        self.formItem.processing_method = response.data.processing_method
    ...
    save () {
      ...
      this.$axios.post(`quality/defectGrade/`, {
        name: self.formItem.name,
        code: self.formItem.code,
        rule: self.formItem.rule,
        influences: self.formItem.influences,
        processing_method: self.formItem.processing_method,
    ...
    update () {
      ...
      this.$axios.put(`quality/defectGrade/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        rule: self.formItem.rule,
        influences: self.formItem.influences,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`quality/defectGrade/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`quality/defectGrade/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  mounted () {
    this.attribute_title = this.$store.getters.getConfig.attach_attribute.缺陷等级
  },
  computed: {
    username () {
    ...
    canCreate () {
      return 'quality.add_defectgradedefinitionmodel' in this.permissions
    },
    canRead () {
      return 'quality.read_defectgradedefinitionmodel' in this.permissions
    }
