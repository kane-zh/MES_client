<template>
  <div class="inspectionStandardType">
    <!-- 列表页显示-->
      ...
      <div v-if="showTree===false">
        <div class="listHead">
          ...
          <div class="button" >
            ...
            <button type="button" @click="showCreatView"  v-show="canCreate===true">添加检验标准类型</button>
          </div>
          <div class="ordering">
            <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"检验标准类型名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"检验标准类型编码:"+"&#12288;"+detail.code}}</li>
        ...
        <Collapse active-key="2" accordion v-if="detail.inspectionStandardType_child!==undefined && detail.inspectionStandardType_child.length > 0">
          子类别:
          <table >
            <tr align="center"  type="height:2em">
              <th>序号</th>
            ...
            <tr align="center" v-for="(item,index) in detail.inspectionStandardType_child" :key="item.id" type="height:1em" >
              <td>{{index}}</td>
              <td>{{item.name}}</td>
              <td>{{item.code}}</td>
              <td>{{item.classes}}</td>
        ...
        <Collapse active-key="3" accordion v-if="detail.inspectionStandardType_item!==undefined && detail.inspectionStandardType_item.length > 0">
          包含内容:
          <Panel >
            <div  slot="content">
              <table >
                ...
                <tr align="center" v-for="(item,index) in detail.inspectionStandardType_item" :key="item.id" type="height:1em" >
                  <td>{{index}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.code}}</td>
                  <td>{{item.state}}</td>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
          <div>类型名称:
            <input v-model="formItem.name"  placeholder="请输入检验标准类型名称...">
          </div>
          <div>类型编码:
            <input v-model="formItem.code"  placeholder="请输入检验标准类型编码...">
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('quality/inspectionStandardType/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('quality/inspectionStandardType/?state=' + self.selectItem.state +
    ...
    showListTreeView: function () {
      this.treeData = []
      var self = this
      this.$axios.get('quality/inspectionStandardTypes/').then(function (response) {
    ...
    transformToTree (data) {
      ...
      for (let l1 = 0; l1 < data.length; l1++) {
        /* 遍历第二层,给第一层子项添加数据 */
        let data2 = data[l1].inspectionStandardType_child
        let children1 = []
        for (let l2 = 0; l2 < data2.length; l2++) {
          /* 遍历第三层,给第二层子项添加数据 */
          let data3 = data2[l2].inspectionStandardType_child
          let children2 = []
          for (let l3 = 0; l3 < data3.length; l3++) {
            /* 遍历第四层,给第三层子项添加数据 */
            let data4 = data3[l3].inspectionStandardType_child
            let children3 = []
    ...
    getTreeIndex () {
      ...
      if (!(('quality.read_inspectionstandardtypedefinitionmodel' in this.permissions) || (data[0].auditor === this.username) || (data[0].create_user === this.username))) {
        alert('当前账号不具备查看详情权限')
        return
      }
      if (data[0].state !== '使用中') {
    ...
    showDetailView (id) {
      ...
      this.parent = {}
      var self = this
      this.$axios.get(`quality/inspectionStandardType/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`quality/inspectionStandardType/` + self.detail.id + '/', {
    ...
    save () {
      ...
      this.$axios.post(`quality/inspectionStandardType/`, {
    ...
    update () {
      ...
      this.$axios.put(`quality/inspectionStandardType/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        classes: self.formItem.classes,
        parent: self.formItem.parent,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`quality/inspectionStandardType/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`quality/inspectionStandardType/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  mounted () {
    this.attach_attributes = this.$store.getters.getConfig.attach_attribute.检验标准
  },
  computed: {
    username () {
    ...
    canCreate () {
      return 'quality.add_inspectionstandardtypedefinitionmodel' in this.permissions
    },
    canRead () {
      return 'quality.read_inspectionstandardtypedefinitionmodel' in this.permissions
    }
    ...
    'formItem.classes': function (newval, oldval) {
      ...
      switch (newval) {
        case '一级类别':
          ...
          break
        case '二级类别':
          this.$axios.get('quality/inspectionStandardType/?state=使用中&classes=一级类别&auditor=&create_user=&page_size=99999&ordering=-id').then(function (response) {
            self.list = response.data.results
          }).catch(function (err) {
            if (err.request) {
              alert(err.request.response)
          ...
          break
        case '三级类别':
          this.$axios.get('quality/inspectionStandardType/?state=使用中&classes=二级类别&auditor=&create_user=&page_size=99999&ordering=-id').then(function (response) {
            self.list = response.data.results
          }).catch(function (err) {
            if (err.request) {
              alert(err.request.response)
          ...
          break
        case '四级类别':
          this.$axios.get('quality/inspectionStandardType/?state=使用中&classes=三级类别&auditor=&create_user=&page_size=99999&ordering=-id').then(function (response) {
            self.list = response.data.results
          }).catch(function (err) {
            if (err.request) {
              alert(err.request.response)
