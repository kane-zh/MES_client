          ...
          <form class="select">
            ...
            <div>产品分类:
              <select v-model="selectItem.productType" placeholder="请选择产品分类"      >
                <option v-for="item in productTypeInfor" :value="item.code" :key="item.code">{{item.name}}</option>
              </select>
            </div>
            <div>排序:
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/productStockInfor/?ordering=' + self.selectItem.ordering).then(function (response) {
    ...
    showDetailView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/productStockDetail/?ordering=' + self.selectItem.ordering).then(function (response) {
        self.list = response.data.results
        self.listCount = response.data.count
        if (response.data.next !== null) {
          self.listNextUrl = response.data.next.replace(self.$axios.defaults.baseURL, '')
    ...
    select_detail () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/productStockDetail/?warehouse_code=' + self.selectItem.warehouse +
              '&productType_code=' + self.selectItem.productType +
    ...
    showCreatView (value) {
      ...
      this.formItem.product = parseInt(value.product_id)
  ...
  created () {
    ...
    this.$axios.get('warehouse/warehouse/?page_size=99999&ordering=-id&type=产品库&state=使用中').then(function (response) {
      ...
      self.$axios.get('process/productType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
        self.productTypeInfor = response.data.results
