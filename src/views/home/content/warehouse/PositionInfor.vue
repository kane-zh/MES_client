  ...
  <div class="positionInfor">
    ...
    <div  class="list">
      <div class="heard">
        <form class="select">
          ...
          <div>分类:
            <select v-model="selectItem.type">
              <option v-for="item in warehouseInfor" :value="item.id" :key="item.id">{{item.code+"("+item.name+")"}}</option>
            </select>
          </div>
          <div>排序:
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加库位信息</button>
        </div>
      </div>
      <div class="content">
        <div class="table">
       ...
       <div class="heard">
        ...
        <div class="content">
          <div class="basic">
            ...
            <dd>{{"仓库:"+"&#12288;"+type.code+"("+type.name+")"}}</dd>
            <dd>{{"最大容量:"+"&#12288;"+detail.maximum}}</dd>
            <dd>{{"位置:"+"&#12288;"+detail.place}}</dd>
        ...
        <div class="button">
          ...
          <button type="button" @click="changeState('闲置')" v-show="showApprovedBt===true" >通过审核</button>
        ...
        <div class="content">
          <form >
              ...
              <div>父仓库:
                ...
                <span class="message" v-show ="!$v.formItem.type.required">请选择仓库</span>
              </div>
              <div>最大容量:
                <input v-model="formItem.maximum"  type="number"  placeholder="请输入最大容量..." >
              </div>
              <div>位置:
                <input v-model="formItem.place"  placeholder="请输入位置...">
              </div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/position/?ordering=' + self.ordering).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`warehouse/position/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`warehouse/position/` + id).then(function (response) {
        ...
        self.formItem.maximum = response.data.maximum
        self.formItem.place = response.data.place
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`warehouse/position/`, {
        ...
        self.formItem.image = []
        self.imageData = []
        self.$axios.patch(`warehouse/position/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  computed: {
    ...
    canCreate () {
      return 'warehouse.add_positiondefinitionmodel' in this.permissions
    },
    canRead () {
      return 'warehouse.read_positiondefinitionmodel' in this.permissions
    }
