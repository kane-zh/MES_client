  ...
  <div class="productManage">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加产品管理</button>
        </div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          ...
          <li>{{"物料分类:"+"&#12288;"+detail.productType_name+"("+productType_code+")"}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        ...
        <div>产品分类:
          <select v-model="formItem.productType"   placeholder="请选择产品分类"      :disabled="inputDisable">
            <option v-for="item in productType" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
        </div>
        <div>产品:
          <select v-model="formItem.product"   placeholder="请选择产品"     :disabled="inputDisable">
            <option v-for="item in productInfor" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
          <span class="message" v-if="!$v.formItem.product.required">请选择产品</span>
        </div>
        <div>批次:
    ...
    <div  class="update"  v-show="showViewid==='update'">
      <form >
        ...
        <div>产品分类:
          <select v-model="formItem.productType"   placeholder="请选择产品分类"      :aria-disabled="inputDisable">
            <option v-for="item in productType" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
        </div>
        <div>产品:
          <select v-model="formItem.product"   placeholder="请选择产品"     :aria-disabled="inputDisable">
            <option v-for="item in productInfor" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
          <span class="message" v-if="!$v.formItem.product.required">请选择产品</span>
        </div>
        <div>批次:
          <input v-model="formItem.batch"  placeholder="请输入批次.." :aria-disabled="inputDisable">
        </div>
        <div>操作者:
          <input v-model="formItem.handler"  placeholder="请输入操作者...">
        </div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/productManage/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/productManage/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      var self = this
      this.$axios.get(`warehouse/productManage/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`warehouse/productManage/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`warehouse/productManage/` + id).then(function (response) {
        ...
        self.formItem.product = parseInt(response.data.product_id)
        ...
        for (var i = 0; i < self.productType.length; i++) {
          if (self.productType[i].code === response.data.productType_code) {
            self.formItem.productType = self.productType[i].id
            break
          }
        }
    ...
    save () {
      ...
      this.$axios.post(`warehouse/productManage/`, {
    ...
    update () {
      ...
      this.$axios.put(`warehouse/productManage/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        batch: self.formItem.batch,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`warehouse/productManage/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`warehouse/productManage/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          if (self.formItem.type === '入库操作') {
  ...
  computed: {
    ...
    canCreate () {
      return 'warehouse.add_productmanagemodel' in this.permissions
    },
    canRead () {
      return 'warehouse.read_productmanagemodel' in this.permissions
    }
  ...
  watch: {
    ...
    'formItem.type': function (newval, oldval) {
      if ((newval === '入库操作') || (newval === '退库操作')) {
        ...
        var self = this
        this.$axios.get('process/productInfor/?page_size=99999&ordering=-id').then(function (response) {
    ...
    'formItem.position': function (newval, oldval) {
      ...
      if ((this.formItem.type !== '入库操作') && (this.formItem.type !== '退库操作')) {
        ...
        this.$axios.get('warehouse/productStockDetail/?warehouse_code=' + warehouseCode +
          ...
          response.data.results.forEach(function (value, i) {
            if (newval === parseInt(value.position_id) && (value.state === '使用中')) {
              for (var j = 0; j < self.productType.length; j++) {
                if (self.productType[j].code === value.productType_code) {
                  self.formItem.productType = self.productType[j].id
                  self.formItem.product = parseInt(value.product_id)
