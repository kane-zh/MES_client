        ...
        <form class="select">
            ...
            <div>半成品类型:
              <select v-model="selectItem.semifinishedType" placeholder="请选择半成品类型"      >
                <option v-for="item in semifinishedTypeInfor" :value="item.code" :key="item.code">{{item.name}}</option>
              </select>
            </div>
          <div>关键字:
          ...
          <div>
            <button type="button" @click="showListView" style="background: #FCC400;border: none">重置</button>
          </div>
        </form>
        <div class="ordering">
          <div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/semifinishedStockInfor/?ordering=' + self.ordering).then(function (response) {
    ...
    select_list () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/semifinishedStockInfor/?warehouse_code=' + self.selectItem.warehouse +
    ...
    showDetailView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/semifinishedStockDetail/?ordering=' + self.ordering).then(function (response) {
        self.list = response.data.results
        self.listCount = response.data.count
        if (response.data.next !== null) {
          self.listNextUrl = response.data.next.replace(self.$axios.defaults.baseURL, '')
    ...
    select_detail () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/semifinishedStockDetail/?warehouse_code=' + self.selectItem.warehouse +
              '&semifinishedType_code=' + self.selectItem.semifinishedType +
    ...
    showCreatView (value) {
      ...
      this.formItem.semifinished = parseInt(value.semifinished_id)
  ...
  created () {
    var self = this
    this.$axios.get('warehouse/warehouse/?page_size=99999&ordering=-id&type=半成品库&state=使用中').then(function (response) {
      ...
      self.$axios.get('process/semifinishedType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
        self.semifinishedTypeInfor = response.data.results
