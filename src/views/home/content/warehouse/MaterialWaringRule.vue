      ...
      <div class="center">
        ...
        <div class="content">
          <form>
            ...
            <div>物料信息:
              <select v-model="formItem_child.material" >
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/materialWaringRule/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/materialWaringRule/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      this.list_child = []
      var self = this
      this.$axios.get(`warehouse/materialWaringRule/` + id).then(function (response) {
        ...
        response.data.child.forEach(function (value, i) {
          var obj1 = {'id': value.id,
            ...
            'materialTypeCode': value.materialType_code,
            'materialTypeName': value.materialType_name,
    ...
    changeState (state) {
      ...
      this.$axios.patch(`warehouse/materialWaringRule/` + self.detail.id + '/', {
    ...
    uploadChild () {
      var self = this
      this.$axios.post(`warehouse/materialWaringRuleItem/`, {
        warehouse_code: self.formItem_child.warehouse,
        material_id: self.formItem_child.material,
        ...
        for (var i = 0; i < self.materialInfor.length; i++) {
          if (self.materialInfor[i].id === parseInt(response.data.material_id)) {
            index = i
            break
          }
        }
        ...
        for (var j = 0; j < self.materialType.length; j++) {
          if (self.materialType[j].id === self.formItem_child.materialType) {
            index1 = j
            break
          }
        }
        ...
        self.formItem_child.materialType = ''
        self.formItem_child.material = ''
    ...
    save () {
      ...
      this.$axios.post(`warehouse/materialWaringRule/`, {
    ...
    update () {
      ...
      this.$axios.put(`warehouse/materialWaringRule/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        child: self.formItem.child,
        file: self.formItem.file,
    ...
    saveAndSubmit () {
      ...
      this.$axios.post(`warehouse/materialWaringRule/`, {
        ...
        self.formItem.file = []
        self.fileData = []
        self.$axios.patch(`warehouse/materialWaringRule/` + response.data.id + '/', {
          state: '审核中'
        }).then(function (response
        ) {
          alert('数据提交成功')
  ...
  created () {
    ...
    this.$axios.get('user/userInfor/?page_size=99999&ordering=-id').then(function (response) {
      ...
      self.$axios.get('process/materialType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
        ...
        self.$axios.get('warehouse/warehouse/?page_size=99999&ordering=-id&type=物料库&state=使用中').then(function (response) {
  ...
  mounted () {
    this.attribute_title = this.$store.getters.getConfig.attach_attribute.物料库存预警规则
    this.attribute_title_child = this.$store.getters.getConfig.attach_attribute.物料库存预警规则子项
  },
  computed: {
    username () {
    ...
    canCreate () {
      return 'warehouse.add_materialwaringrulemodel' in this.permissions
    },
    canRead () {
      return 'warehouse.read_materialwaringrulemodel' in this.permissions
    }
  ...
  watch: {
    ...
    'formItem_child.materialType': function (newval, oldval) {
      var self = this
      this.materialInfor = []
