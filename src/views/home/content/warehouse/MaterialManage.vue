<template>
  <div class="materialManage">
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"物料管理名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"物料管理编码:"+"&#12288;"+detail.code}}</li>
          ...
          <li>{{"物料类型编码:"+"&#12288;"+detail.materialType_code}}</li>
          <li>{{"物料类型名称:"+"&#12288;"+detail.materialType_name}}</li>
          <li>{{"物料编码:"+"&#12288;"+detail.material_code}}</li>
          <li>{{"物料名称:"+"&#12288;"+detail.material_name}}</li>
    ...
    <div  class="update"  v-show="showViewid==='update'">
      <form >
        ...
        <div>物料:
          <select v-model="formItem.material"   placeholder="请选择物料"     :aria-disabled="inputDisable">
            <option v-for="item in materialInfor" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
          <span class="message" v-if="!$v.formItem.material.required">请选择物料</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/materialManage/?ordering=' + self.ordering).then(function (response) {
    ...
    showDetailView (id) {
      ...
      var self = this
      this.$axios.get(`warehouse/materialManage/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`warehouse/materialManage/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`warehouse/materialManage/` + id).then(function (response) {
        ...
        self.formItem.material = parseInt(response.data.material_id)
        ...
        for (var i = 0; i < self.materialType.length; i++) {
          if (self.materialType[i].code === response.data.materialType_code) {
            self.formItem.materialType = self.materialType[i].id
            break
          }
        }
    ...
    update () {
      ...
      this.$axios.put(`warehouse/materialManage/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        batch: self.formItem.batch,
  ...
  computed: {
    ...
    canCreate () {
      return 'warehouse.add_materialmanagemodel' in this.permissions
    },
    canRead () {
      return 'warehouse.read_materialmanagemodel' in this.permissions
    }
  ...
  watch: {
    ...
    'formItem.type': function (newval, oldval) {
      if ((newval === '入库操作') || (newval === '退库操作')) {
        ...
        this.materialInfor = []
        this.formItem.material = ''
        this.formItem.materialType = ''
        this.formItem.batch = ''
      } else if ((newval === '增加操作') || (newval === '出库操作') || (newval === '盘点操作')) {
        this.showKey = '使用中'
        this.inputDisable = true
        var self = this
        this.$axios.get('process/materialInfor/?page_size=99999&ordering=-id').then(function (response) {
          ...
          response.data.results.forEach(function (value, i) {
            if (newval === parseInt(value.position_id) && (value.state === '使用中')) {
              for (var j = 0; j < self.materialType.length; j++) {
                if (self.materialType[j].code === value.materialType_code) {
                  self.formItem.materialType = self.materialType[j].id
                  self.formItem.material = parseInt(value.material_id)
