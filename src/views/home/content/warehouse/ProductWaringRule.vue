<template>
  <div class="productWaringRule">
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"产品库存预警名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"产品库存预警编码:"+"&#12288;"+detail.code}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>产品预警规则名称:
          <input v-model="formItem.name"  placeholder="请输入产品预警规则名称...">
        </div>
        <div>产品预警规则编码:
          <input v-model="formItem.code"  placeholder="请输入产品预警规则编码...">
        ...
        <table v-show="list_child.length>0">
          ...
          <tr align="center" v-for="(item,index) in list_child" :key="item.id" type="height:1em" >
            ...
            <td>{{item.producTypeName}}</td>
            <td>{{item.productTypCode}}</td>
