  ...
  <div class="partsManage">
    ...
    <div  class="list"  v-if="showViewid==='list'">
      <div class="listHead">
        ...
        <div class="button" >
          <button type="button" @click="showCreatView"  v-show="canCreate===true">添加备品管理</button>
        </div>
        <div class="ordering">
          <div>
    ...
    <div  class="detail"  v-show="showViewid==='detail'">
      <div class="content">
        <ul>
          <li>{{"备品管理名称:"+"&#12288;"+detail.name}}</li>
          <li>{{"备品管理编码:"+"&#12288;"+detail.code}}</li>
          ...
          <li>{{"备品类型编码:"+"&#12288;"+detail.partsType_code}}</li>
          <li>{{"备品类型名称:"+"&#12288;"+detail.partsType_name}}</li>
          <li>{{"备品编码:"+"&#12288;"+detail.parts_code}}</li>
          <li>{{"备品名称:"+"&#12288;"+detail.parts_name}}</li>
          <li>{{"操作者:"+"&#12288;"+detail.handler}}</li>
          <li>{{"数量:"+"&#12288;"+detail.sum}}</li>
          <li>{{"操作时间:"+"&#12288;"+detail.dataTime}}</li>
    ...
    <div  class="create"  v-show="showViewid==='create'">
      <form >
        <div>备品管理名称:
          <input v-model="formItem.name"  placeholder="请输入备品管理名称...">
        </div>
        <div>备品管理编码:
          <input v-model="formItem.code"  placeholder="请输入备品管理编码...">
        </div>
        <div>操作类型:
        ...
        <div>仓位:
          <select v-model="formItem.position"   placeholder="请选择仓位">
            <option v-for="item in positionInfor" :value="item.id" :key="item.id" v-if="item.state===showKey">{{item.name+"("+item.code+")"}}</option>
          </select>
          <span class="message" v-if="!$v.formItem.position.required">请选择仓位</span>
        </div>
        <div>备品类型:
          <select v-model="formItem.partsType"   placeholder="请选择备品类型"      :disabled="inputDisable">
            <option v-for="item in partsType" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
        </div>
        <div>备品:
          <select v-model="formItem.parts"   placeholder="请选择备品"     :disabled="inputDisable">
            <option v-for="item in partsInfor" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
          <span class="message" v-if="!$v.formItem.parts.required">请选择备品</span>
        </div>
        <div>操作者:
    ...
    <div  class="update"  v-show="showViewid==='update'">
      <form >
        ...
        <div>备品类型:
          <select v-model="formItem.partsType"   placeholder="请选择备品类型"      :aria-disabled="inputDisable">
            <option v-for="item in partsType" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
        </div>
        <div>备品:
          <select v-model="formItem.parts"   placeholder="请选择备品"     :aria-disabled="inputDisable">
            <option v-for="item in partsInfor" :value="item.id" :key="item.id">{{item.name+"("+item.code+")"}}</option>
          </select>
          <span class="message" v-if="!$v.formItem.parts.required">请选择备品</span>
        </div>
        <div>数量:
          <input-number v-model="formItem.sum" :min="0"  ></input-number>
        </div>
        <div>操作者:
          <input v-model="formItem.handler"  placeholder="请输入操作者...">
        </div>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/partsManage/?ordering=' + self.ordering).then(function (response) {
    ...
    select () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/partsManage/?state=' + self.selectItem.state +
    ...
    showDetailView (id) {
      ...
      var self = this
      this.$axios.get(`warehouse/partsManage/` + id).then(function (response) {
    ...
    changeState (state) {
      ...
      this.$axios.patch(`warehouse/partsManage/` + self.detail.id + '/', {
    ...
    showUpdateView (id) {
      ...
      this.$axios.get(`warehouse/partsManage/` + id).then(function (response) {
        ...
        self.formItem.parts = parseInt(response.data.parts_id)
        ...
        for (var i = 0; i < self.partsType.length; i++) {
          if (self.partsType[i].code === response.data.partsType_code) {
            self.formItem.partsType = self.partsType[i].id
            break
          }
        }
    ...
    update () {
      ...
      this.$axios.put(`warehouse/partsManage/` + self.formItem.id + '/', {
        name: self.formItem.name,
        code: self.formItem.code,
        type: self.formItem.type,
        handler: self.formItem.handler,
  ...
  created () {
    ...
    this.$axios.get('user/userInfor/?page_size=99999&ordering=-id').then(function (response) {
      ...
      self.$axios.get('warehouse/warehouse/?page_size=99999&ordering=-id&type=备品库&state=使用中').then(function (response) {
  ...
  computed: {
    ...
    canRead () {
      return 'warehouse.read_partsmanagemodel' in this.permissions
    }
  ...
  watch: {
    ...
    'formItem.partsType': function (newval, oldval) {
      ...
      this.$axios.get(`equipment/partsType/` + newval).then(function (response) {
        self.partsInfor = response.data.partsType_item
      }).catch(function (err) {
        if (err.request) {
          alert(err.request.response)
        } else {
    ...
    'formItem.type': function (newval, oldval) {
      if ((newval === '入库操作') || (newval === '退库操作')) {
        ...
        this.partsInfor = []
        this.formItem.parts = ''
        this.formItem.partsType = ''
      } else if ((newval === '增加操作') || (newval === '出库操作') || (newval === '盘点操作')) {
        this.showKey = '使用中'
        this.inputDisable = true
        var self = this
        this.$axios.get('equipment/partsInfor/?page_size=99999&ordering=-id').then(function (response) {
    ...
    'formItem.position': function (newval, oldval) {
      ...
      if ((this.formItem.type !== '入库操作') && (this.formItem.type !== '退库操作')) {
        ...
        this.$axios.get('warehouse/partsStockDetail/?warehouse_code=' + warehouseCode +
          ...
          response.data.results.forEach(function (value, i) {
            if (newval === parseInt(value.position_id) && (value.state === '使用中')) {
              for (var j = 0; j < self.partsType.length; j++) {
                if (self.partsType[j].code === value.partsType_code) {
                  self.formItem.partsType = self.partsType[j].id
                  self.formItem.parts = parseInt(value.parts_id)
                  break
                }
              }
            }
