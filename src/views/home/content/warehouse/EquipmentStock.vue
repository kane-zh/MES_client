        ...
        <div class="table">
          <table >
            ...
            <tr align="center" v-for="(item,index) in list" :key="item.id" type="height:1em" >
              ...
              <td>{{item.equipmentType_name+"("+item.equipmentType_code+")"}}</td>
              <td>{{item.equipment_name+"("+item.equipment_code+")"}}</td>
              <td>{{item.sum}}</td>
            </tr>
            <tr>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/equipmentStockInfor/?ordering=' + self.selectItem.ordering).then(function (response) {
    ...
    showDetailView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/equipmentStockDetail/?ordering=' + self.selectItem.ordering).then(function (response) {
        self.list = response.data.results
        self.listCount = response.data.count
        if (response.data.next !== null) {
          self.listNextUrl = response.data.next.replace(self.$axios.defaults.baseURL, '')
    ...
    select_detail () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/equipmentStockDetail/?warehouse_code=' + self.selectItem.warehouse +
              '&equipmentType_code=' + self.selectItem.equipmentType +
    ...
    showCreatView (value) {
      ...
      this.formItem.equipment = parseInt(value.equipment_id)
  ...
  created () {
    var self = this
    this.$axios.get('warehouse/warehouse/?page_size=99999&ordering=-id&type=设备库&state=使用中').then(function (response) {
      ...
      self.$axios.get('equipment/equipmentType/?page_size=99999&ordering=-id&state=使用中').then(function (response) {
        self.equipmentTypeInfor = response.data.results
  ...
  mounted () {
    this.attribute_title = this.$store.getters.getConfig.attach_attribute.设备管理
  }
}
</script>
