    ...
    <div  class="list" v-show="showViewid==='list'">
      <div class="heard">
        <form class="select">
          ...
          <div>备品分类:
            <select v-model="selectItem.partsType" placeholder="请选择备品分类"      >
              <option v-for="item in partsTypeInfor" :value="item.code" :key="item.code">{{item.name}}</option>
            </select>
          </div>
          <div>排序:
        ...
        <div class="table">
          <table >
            ...
            <tr align="center" v-for="(item,index) in list" :key="item.id" type="height:1em" >
              ...
              <td>{{item.partsType_name+"("+item.partsType_code+")"}}</td>
...
export default {
  ...
  methods: {
    ...
    showListView: function () {
      ...
      var self = this
      this.$axios.get('warehouse/partsStockInfor/?ordering=' + self.selectItem.ordering).then(function (response) {
    ...
    select_list () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/partsStockInfor/?warehouse_code=' + self.selectItem.warehouse +
    ...
    select_detail () {
      ...
      this.listNextUrl = ''
      var self = this
      this.$axios.get('warehouse/partsStockDetail/?warehouse_code=' + self.selectItem.warehouse +
              '&partsType_code=' + self.selectItem.partsType +
